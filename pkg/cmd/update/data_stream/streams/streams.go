...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
		$ azion update streams --stream-id 1234 --file "update.json"
        `),
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("stream-id") {
				answer, err := utils.AskInput(msg.UpdateAskStreamID)
				if err != nil {
			...
			if !cmd.Flags().Changed("file") {
				answer, err := utils.AskInput(msg.UpdateAskStreamFile)
				if err != nil {
					logger.Debug("Error while parsing answer", zap.Error(err))
					return utils.ErrorParseResponse
				}
			...
			if err := utils.FlagFileUnmarshalJSON(fields.InPath, &request.PatchedDataStreamRequest); err != nil {
			...
			if err != nil {
				return fmt.Errorf(msg.ErrorUpdateStream.Error(), err)
			}
...
func addFlags(flags *pflag.FlagSet, fields *Fields) {
	flags.Int64Var(&fields.ID, "stream-id", 0, msg.FlagID)
