package templates

import (
	"context"
	"encoding/json"
	"fmt"
	"os"
	"strconv"

	"github.com/MakeNowJust/heredoc"
	msg "github.com/aziontech/azion-cli/messages/templates"
	api "github.com/aziontech/azion-cli/pkg/api/data_stream"
	"github.com/aziontech/azion-cli/pkg/cmdutil"
	"github.com/aziontech/azion-cli/pkg/logger"
	"github.com/aziontech/azion-cli/pkg/output"
	"github.com/aziontech/azion-cli/pkg/registry"
	"github.com/aziontech/azion-cli/utils"
	"github.com/spf13/cobra"
	"github.com/spf13/pflag"
	"go.uber.org/zap"
)

type Fields struct {
	ID      int64
	Name    string
	DataSet string
	Active  string
	InPath  string
}

func NewCmd(f *cmdutil.Factory) *cobra.Command {
	fields := &Fields{}

	cmd := &cobra.Command{
		Use:           msg.Usage,
		Short:         msg.UpdateShortDescription,
		Long:          msg.UpdateLongDescription,
		SilenceUsage:  true,
		SilenceErrors: true,
		Example: heredoc.Doc(`
		$ azion update data-stream templates --template-id 1234 --name "New Name"
		$ azion update data-stream templates --template-id 1234 --data-set "./data_set.json" --active false
		$ azion update data-stream templates --template-id 1234 --file "update.json"
        `),
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("template-id") {
				answer, err := utils.AskInput(msg.UpdateAskTemplateID)
				if err != nil {
					logger.Debug("Error while parsing answer", zap.Error(err))
					return utils.ErrorParseResponse
				}

				num, err := strconv.ParseInt(answer, 10, 64)
				if err != nil {
					logger.Debug("Error while converting answer to int64", zap.Error(err))
					return msg.ErrorConvertTemplateId
				}

				fields.ID = num
			}

			request := api.NewUpdateTemplateRequest()

			if cmd.Flags().Changed("file") {
				if err := utils.FlagFileUnmarshalJSON(fields.InPath, &request.PatchedTemplateRequest); err != nil {
					logger.Debug("Failed to unmarshal file", zap.Error(err))
					return utils.ErrorUnmarshalReader
				}
			} else {
				if !cmd.Flags().Changed("name") &&
					!cmd.Flags().Changed("data-set") &&
					!cmd.Flags().Changed("active") {
					return msg.ErrorUpdateNoFields
				}

				if cmd.Flags().Changed("name") {
					request.SetName(fields.Name)
				}

				if cmd.Flags().Changed("data-set") {
					dataSet, err := os.ReadFile(fields.DataSet)
					if err != nil {
						logger.Debug("Failed to read the data set file", zap.Error(err))
						return msg.ErrorDataSetFlag
					}
					if !json.Valid(dataSet) {
						return msg.ErrorParseDataSet
					}
					request.SetDataSet(string(dataSet))
				}

				if cmd.Flags().Changed("active") {
					isActive, err := strconv.ParseBool(fields.Active)
					if err != nil {
						return fmt.Errorf("%w: %q", msg.ErrorActiveFlag, fields.Active)
					}
					request.SetActive(isActive)
				}
			}

			client := registry.DataStream(f)

			ctx := context.Background()
			response, err := client.UpdateTemplate(ctx, request, fields.ID)
			if err != nil {
				return fmt.Errorf(msg.ErrorUpdateTemplate.Error(), err)
			}

			updateOut := output.GeneralOutput{
				Msg:   fmt.Sprintf(msg.UpdateOutputSuccess, response.GetId()),
				Out:   f.IOStreams.Out,
				Flags: f.Flags,
			}
			return output.Print(&updateOut)
		},
	}

	flags := cmd.Flags()
	addFlags(flags, fields)

	return cmd
}

func addFlags(flags *pflag.FlagSet, fields *Fields) {
	flags.Int64Var(&fields.ID, "template-id", 0, msg.FlagID)
	flags.StringVar(&fields.Name, "name", "", msg.FlagName)
	flags.StringVar(&fields.DataSet, "data-set", "", msg.FlagDataSet)
	flags.StringVar(&fields.Active, "active", "true", msg.FlagActive)
	flags.StringVar(&fields.InPath, "file", "", msg.UpdateFlagFile)
	flags.BoolP("help", "h", false, msg.UpdateHelpFlag)
}
