...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
        $ azion update firewall-instance --firewall-id 1234 --instance-id 4321 --name firewallInstanceName
        $ azion update firewall-instance --firewall-id 1234 --instance-id 4321 --name withargs --active true
        $ azion update firewall-instance --firewall-id 1234 --instance-id 4321 --file "update.json"
        `),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			response, err := client.Update(context.Background(), fields.FirewallID, fields.InstanceID, request)
			if err != nil {
