...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
		$ azion update waf-exceptions --waf-id 1234 --exception-id 4321 --name "My Exception"
		$ azion update waf-exceptions --waf-id 1234 --exception-id 4321 --name "My Exception" --active true
		$ azion update waf-exceptions --waf-id 1234 --exception-id 4321 --file "update.json"
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !cmd.Flags().Changed("exception-id") {
				answer, err := utils.AskInput(msg.AskInputExceptionID)
				if err != nil {
					return err
				}
				...
				fields.ExceptionID = num
			}

			var request sdk.PatchedWAFRuleRequest
	...
	flags.Int64Var(&fields.ExceptionID, "exception-id", 0, msg.FlagExceptionID)
