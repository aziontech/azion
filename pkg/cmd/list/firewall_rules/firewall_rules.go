...
import (
	...
	msg "github.com/aziontech/azion-cli/messages/list/firewall_rules"
...
type ListCmd struct {
	...
	ListRules  func(ctx context.Context, opts *contracts.ListOptions, firewallID int64) (*sdk.PaginatedFirewallRuleList, error)
	FirewallID int64
}
...
func NewCobraCmd(list *ListCmd, f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
			$ azion list firewall-rule
			$ azion list firewall-rule --firewall-id 16736354321 --details
			$ azion list firewall-rule --firewall-id 16736354321 --order-by "name"
			$ azion list firewall-rule --firewall-id 16736354321 --order-by "-name"
			$ azion list firewall-rule --firewall-id 16736354321 --page 1
			$ azion list firewall-rule --firewall-id 16736354321 --page-size 5
		`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			if !cmd.Flags().Changed("firewall-id") {
				answer, err := list.AskInput(msg.AskInputFirewallId)
				if err != nil {
					return err
				}
...
func PrintTable(cmd *cobra.Command, list *ListCmd, f *cmdutil.Factory, opts *contracts.ListOptions) error {
	...
	resp, err := list.ListRules(ctx, opts, list.FirewallID)
	if err != nil {
		return err
	}

	listOut := output.ListOutput{}
	listOut.Columns = []string{"ID", "NAME", "ACTIVE", "DESCRIPTION"}
	listOut.Out = f.IOStreams.Out
	listOut.Flags = f.Flags

	if opts.Details {
		listOut.Columns = []string{"ID", "NAME", "ACTIVE", "DESCRIPTION", "LAST EDITOR", "LAST MODIFIED"}
	}
