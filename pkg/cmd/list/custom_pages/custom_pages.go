...
type ListCmd struct {
	...
	ListCustomPages func(context.Context, *contracts.ListOptions) (*sdk.PaginatedCustomPageList, error)
}

func NewListCmd(f *cmdutil.Factory) *ListCmd {
	return &ListCmd{
		Io: f.IOStreams,
		ListCustomPages: func(ctx context.Context, opts *contracts.ListOptions) (*sdk.PaginatedCustomPageList, error) {
...
func NewCobraCmd(list *ListCmd, f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
			$ azion list custom-pages --details
			$ azion list custom-pages --order-by "id"
			$ azion list custom-pages --order-by "name"
			$ azion list custom-pages --order-by "-name"
			$ azion list custom-pages --page 1
			$ azion list custom-pages --page-size 5
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := PrintTable(cmd, f, list, opts); err != nil {
				return fmt.Errorf(msg.ErrorGetCustomPages.Error(), err)
			}
			return nil
		},
	}
...
func PrintTable(cmd *cobra.Command, f *cmdutil.Factory, list *ListCmd, opts *contracts.ListOptions) error {
	...
	customPages, err := list.ListCustomPages(ctx, opts)
	if err != nil {
		return err
	}
	...
	if customPages == nil || len(customPages.Results) == 0 {
		return output.Print(&listOut)
	}
	for _, v := range customPages.Results {
		var ln []string
		if opts.Details {
			ln = []string{
