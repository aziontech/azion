...
type DeleteCmd struct {
	...
	DeleteNetworkList func(context.Context, string) error
...
func NewDeleteCmd(f *cmdutil.Factory) *DeleteCmd {
	return &DeleteCmd{
		...
		DeleteNetworkList: func(ctx context.Context, networkListID string) error {
			client := api.NewClient(f.HttpClient, f.Config.GetString("api_v4_url"), f.Config.GetString("token"))
			return client.Delete(ctx, networkListID)
		},
...
func NewCobraCmd(delete *DeleteCmd, f *cmdutil.Factory) *cobra.Command {
	cobraCmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
		$ azion delete network-list --network-list-id 1234
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
			var err error

			if !cmd.Flags().Changed("network-list-id") {
				answer, err := delete.AskInput(msg.AskNetworkListID)
				if err != nil {
					return err
				}

				networkListID = answer
			}
	...
	cobraCmd.Flags().StringVar(&networkListID, "network-list-id", "", msg.FlagID)
