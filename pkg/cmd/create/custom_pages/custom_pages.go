...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
        $ azion create custom-pages --file "create.json"
        `),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := utils.FlagFileUnmarshalJSON(fields.InPath, &request.CustomPageRequest); err != nil {
			...
			if err != nil {
				return fmt.Errorf(msg.ErrorCreateCustomPage.Error(), err)
			}
