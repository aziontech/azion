...
func TestDescribe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "describe a DNS record",
			request:   httpmock.REST("GET", "workspace/dns/zones/1337/records/1111"),
			response:  httpmock.JSONFromFile("./fixtures/response.json"),
			args:      []string{"--zone-id", "1337", "--record-id", "1111"},
			expectErr: false,
		},
		{
			name:      "describe a DNS record - no ids, ask input",
			request:   httpmock.REST("GET", "workspace/dns/zones/1337/records/1337"),
			response:  httpmock.JSONFromFile("./fixtures/response.json"),
			expectErr: false,
		...
		{
			name:      "not found",
			request:   httpmock.REST("GET", "workspace/dns/zones/1337/records/1234"),
