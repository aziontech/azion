...
func TestDescribe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "describe a DNS zone",
			request:   httpmock.REST("GET", "workspace/dns/zones/1337"),
			response:  httpmock.JSONFromFile("./fixtures/response.json"),
			args:      []string{"--zone-id", "1337"},
			expectErr: false,
		},
		{
			name:      "describe a DNS zone - no zone id, ask input",
			request:   httpmock.REST("GET", "workspace/dns/zones/1337"),
			response:  httpmock.JSONFromFile("./fixtures/response.json"),
			expectErr: false,
		...
		{
			name:      "not found",
			request:   httpmock.REST("GET", "workspace/dns/zones/1234"),
