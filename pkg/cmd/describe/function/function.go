...
type DescribeCmd struct {
	...
	Get        func(context.Context, int64) (sdk.Functions, error)
	FunctionID int64
}
...
func NewDescribeCmd(f *cmdutil.Factory) *DescribeCmd {
	return &DescribeCmd{
		...
		Get: func(ctx context.Context, functionID int64) (sdk.Functions, error) {
			client := registry.Function(f)
...
func NewCobraCmd(describe *DescribeCmd, f *cmdutil.Factory) *cobra.Command {
	...
	cobraCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("function-id") {
				...
				describe.FunctionID = num
			}

			ctx := context.Background()
			resp, err := describe.Get(ctx, describe.FunctionID)
			if err != nil {
	...
	cobraCmd.Flags().Int64Var(&describe.FunctionID, "function-id", 0, msg.FlagID)
