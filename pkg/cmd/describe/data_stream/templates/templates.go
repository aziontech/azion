...
func NewDescribeCmd(f *cmdutil.Factory) *DescribeCmd {
	return &DescribeCmd{
		...
		Get: func(ctx context.Context, templateID int64) (sdk.Template, error) {
			client := registry.DataStream(f)
			return client.GetTemplate(ctx, templateID)
		},
	}
}
...
func NewCobraCmd(describe *DescribeCmd, f *cmdutil.Factory) *cobra.Command {
	...
	cobraCmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
        $ azion describe data-stream templates --template-id 4312
        $ azion describe data-stream templates --template-id 1337 --out "./tmp/test.json" --format json
        $ azion describe data-stream templates --template-id 1337 --format json
        `),
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("template-id") {
				answer, err := describe.AskInput(msg.AskTemplateID)
				if err != nil {
					return err
				}
