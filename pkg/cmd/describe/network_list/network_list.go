...
func NewCobraCmd(describe *DescribeCmd, f *cmdutil.Factory) *cobra.Command {
	...
	cobraCmd := &cobra.Command{
		...
		SilenceErrors: true,
		Example: heredoc.Doc(`
        $ azion describe network-list --network-list-id 4312
        $ azion describe network-list --network-list-id 1337 --with-code
        $ azion describe network-list --network-list-id 1337 --out "./tmp/test.json" --format json
        $ azion describe network-list --network-list-id 1337 --format json
        `),
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("network-list-id") {
				answer, err := describe.AskInput(msg.AskNetworkListID)
				if err != nil {
					return err
				}
