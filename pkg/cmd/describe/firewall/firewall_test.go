...
func TestDescribe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "describe a firewall",
			request:   httpmock.REST("GET", "workspace/firewalls/1337"),
			response:  httpmock.JSONFromFile("./fixtures/response.json"),
			args:      []string{"--firewall-id", "1337"},
			expectErr: false,
		},
		{
			name:      "describe a firewall - no firewall id",
			request:   httpmock.REST("GET", "workspace/firewalls/1337"),
			response:  httpmock.JSONFromFile("./fixtures/response.json"),
			expectErr: false,
		...
		{
			name:      "not found",
			request:   httpmock.REST("GET", "workspace/firewalls/1234"),
