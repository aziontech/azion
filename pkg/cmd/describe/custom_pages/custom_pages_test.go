...
func TestDescribe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "describe a custom page",
			request:   httpmock.REST("GET", "workspace/custom_pages/1337"),
			response:  httpmock.JSONFromString(successResponse),
			args:      []string{"--custom-page-id", "1337"},
			expectErr: false,
		},
		{
			name:      "describe a custom page - no custom page id",
			request:   httpmock.REST("GET", "workspace/custom_pages/1337"),
			response:  httpmock.JSONFromString(successResponse),
			expectErr: false,
		...
		{
			name:      "not found",
			request:   httpmock.REST("GET", "workspace/custom_pages/9999"),
			response:  httpmock.StatusStringResponse(http.StatusNotFound, "Not Found"),
			args:      []string{"--custom-page-id", "9999"},
			expectErr: true,
		},
	}
